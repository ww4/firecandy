  ...
  [:div
   ...
   [:ul
    (for [choicespec choicespecs]
      [:li (str (:label choicespec) " (" (get (choice-totals responses q-code) (:choice-code choicespec) 0)  ")")])]])

(defn qcat [partq suffix]
  (keyword (str (name partq) suffix)))

(defn teacher-comments [responses teacher-name part-code]
  [:div
   [:hr]
   [:h3 teacher-name]
   (choice-matrix responses col-titles-1 choices-1 [{:label "Fun" :q-code (qcat part-code "S1")}
                                                    {:label "Results" :q-code  (qcat part-code "S2")}])
   (answer-list responses "" (symbol (str :Q202 "a"))) (answer-list responses "" (qcat part-code "a"))])
